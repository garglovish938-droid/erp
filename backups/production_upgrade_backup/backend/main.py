...
def export_purchases(
    ...
    import io
    q = db.query(PurchaseOrder, Supplier, InventoryItem).join(
        Supplier, PurchaseOrder.supplier_id == Supplier.id
    ).join(
        InventoryItem, PurchaseOrder.inventory_id == InventoryItem.id
    ).filter(PurchaseOrder.is_deleted == False)
    ...
    rows_raw = q.order_by(PurchaseOrder.created_at.desc()).all()

    headers = ["PO Number", "Date", "Supplier", "Item", "Category", "Qty", "Unit Cost", "Total Cost", "Status"]
    rows = []
    for po, sup, inv in rows_raw:
        rows.append([
            po.po_number,
            str(po.created_at.date()) if po.created_at else "",
            sup.name,
            inv.name,
