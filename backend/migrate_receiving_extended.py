"""
Database Migration Script - Extended Stock Transaction Fields
Adds expiry, mrp, price, and selling_cost to stock_transactions table.
"""
import os
import shutil
import sqlite3
...
def backup_database():
    ...
    backup_path = os.path.join(BACKUP_DIR, f"erp_backup_txn_extended_{timestamp}.db")
...
def migrate():
    ...
    try:
        ...
        add_column_if_missing(cursor, "stock_transactions", "expiry", "DATE")
        add_column_if_missing(cursor, "stock_transactions", "mrp", "REAL DEFAULT 0.0")
        add_column_if_missing(cursor, "stock_transactions", "price", "REAL DEFAULT 0.0")
        add_column_if_missing(cursor, "stock_transactions", "selling_cost", "REAL DEFAULT 0.0")
        conn.commit()
        print("\n=== MIGRATION COMPLETE ===")
    except Exception as e:
        conn.rollback()
